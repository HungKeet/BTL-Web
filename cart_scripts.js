document.addEventListener("DOMContentLoaded", () => {
  ...
  function renderCart() {
    ...
    cart.forEach((item, index) => {
      ...
      const imageUrl =
        item.image || "https://placehold.co/80x80/E0E0E0/333333?text=DT";

      // Updated HTML structure for the new grid layout (no wrapper)
      itemElement.innerHTML = `
            <img src="${imageUrl}" alt="${
        item.name
      }" onerror="this.onerror=null;this.src='https://placehold.co/80x80/E0E0E0/333333?text=DT';">
            ...
            <span class="item-total-price">${itemTotalPrice.toLocaleString(
              "vi-VN"
            )}₫</span>
        `;
    ...
    if (totalDisplay)
      totalDisplay.textContent = `${subtotal.toLocaleString("vi-VN")}₫`;
    updateHeaderCartCount();
  }

  // --- Xử lý sự kiện click trên danh sách giỏ hàng (SỬ DỤNG addEventListener) ---
  if (cartListContainer) {
    // Chỉ thêm listener nếu cartListContainer tồn tại
  ...
  if (userSectionCart && typeof displayUser === "function") {
    try {
      ...
      console.error("Could not call displayUser from script.js:", e);
    }
